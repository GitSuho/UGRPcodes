...
plotinterval_list = [0.1, 0.05, 0.01, 0.005, 0.001];
...
E = E_list(ii);
ecc = 0;
plot_interval = plotinterval_list(jj);
nume_name = numerical_method(ord);

filename = sprintf("OneBody4_E%1.2f_plotinterval%0.3f_ecc0.0_%s.txt", E, plot_interval, nume_name);
figurename = sprintf("OneBody4_E%1.2f_plotinterval%0.3f_ecc0.0_%s.pdf", E, plot_interval, nume_name);
fig1 = figure(1);
...
%fit each dt values
X_dt = Fit_dt(dt, plot_interval ,init, @geo, ord);
P_dt = Fit_dt(dt, plot_interval ,init, @lag, ord);
...
for delta = 0:313
    ...
    X(i, :) = RKn([given_position, X(i-1,3:4)], @geo, X_dt, ord);
    P(i, :) = RKn([given_position, P(i-1,3:4)], @lag, P_dt, ord);
...
for i = 2:length(X)
    fprintf(wfile, format, i-1, given_degpos(i,1), given_degpos(i,2), given_degpos(i,3), ...
        X_error(i,1), X(i,1), X(i,2), X_error(i,2), X_error(i,3), X_dt, ...
        P_error(i,1), P(i,1), P(i,2), P_error(i,2), P_error(i,3), P_dt);
end
